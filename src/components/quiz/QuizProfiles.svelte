<script lang="ts">
  ...
  import {
    ...
    updateWritingStyleName,
    updateWritingStyleAnswers,
    deleteWritingStyle,
  } from "../../lib/personality/api.ts";
  ...
  const loadProfileToEdit =
    (writingStyle: WritingStyleDocument) => async () => {
      currentWritingStyle.set(writingStyle);
      editProfile.set("true");
    };

  const updateProfileName =
    (idx: string | undefined) => async (e: CustomEvent<string>) => {
...
<div class="flex flex-row justify-center m-4">
  ...
  >
    ...
    {#await saveToWritingProfileToDB() then}
      {#key deleteIdx}
        {#await createWritingStyleRows()}
          <p>Loading...</p>
        {:then styles}
          {#if styles.length === 0}
            <p class="w-full mt-4 italic text-center text-white">
              No profiles found
            </p>
          {:else}
                    ...
                    >
                      <tbody>
                        {#each styles as style, index}
                          <tr
                            class="text-center sm:text-left items-center sm:items-left flex flex-col mb-4 sm:table-row border-b border-neutral-200 transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-white/10 dark:hover:bg-neutral-200/15"
                          >
                            <td
                              class="whitespace-nowrap font-bold py-4 min-w-64 text-md"
                            >
                              <InlineInput
                                value={style.name}
                                on:blur={updateProfileName(style.id)}
                              />
                            </td>
                            ...
                            <td
                              class="whitespace-nowrap sm:pl-2 py-2 md:m-auto"
                            >
                              <ClipboardButton textToCopy={style.prompt} />
                            </td>
                            <div>
                              {#if open}
                                <!-- Content will be hidden until the user clicks on the summary button. -->
                                <div
                                  class="absolute top-0 left-0 w-full h-full bg-gray-700/30"
                                ></div>
                                <div
                                  class="top-[30%] bg-white rounded-2xl w-[50%] h-fit mx-auto absolute inset-0 text-gray-600 p-4 py-8"
                                >
                                  <!-- Delete Confirmation Popup -->
                                  <div
                                    class="flex flex-col items-center justify-center"
                                  >
                                    <h2 class="text-xl mb-4 align-center">
                                      Are you sure you want to delete this
                                      profile?
                                    </h2>
                                    <div class="flex flex-row gap-4">
                                      <button
                                        class="text-white rounded justify-center bg-red-500 p-1 px-3"
                                        on:click={deleteProfile}
                                      >
                                        <p>Delete</p>
                                      </button>
                                      <button
                                        class="text-white rounded justify-center bg-gray-500 p-1 px-3"
                                        on:click={closePopup}
                                      >
                                        <p>Cancel</p>
                                      </button>
                            ...
                            <td class="whitespace-nowrap px-2 py-2 md:m-auto">
                              <a
                                class="flex flex-row items-center justify-center bg-gray-500 p-1 rounded gap-2 text-white"
                                href={writingStyleToResultUrl(
                                  style.writingStyle,
                                )}
                                on:click={loadProfileToEdit(style.writingStyle)}
                              >
                                <span>
                                  <svg
                            ...
                            <td
                              class="whitespace-nowrap px-2 py-2 md:m-auto mb-2"
                            >
                              <button
                                class="text-white rounded justify-center bg-red-500 p-1 px-3"
                                on:click={openPopup(style.id)}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 576 512"
