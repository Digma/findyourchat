      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            currentVisibleSectionIndex = sections.findIndex(
...
<div class="flex flex-col items-center justify-center w-full h-screen">
  <div
    id="scrollContainer"
    class="w-full h-screen {currentBackgroundColor} transition-colors duration-1000 delay-50 ease-in-out snap-y snap-mandatory h-screen overflow-y-scroll hide-scrollbar"
  >
    <div class="bg-gradient-to-r from-gray-400/0 from-50% to-white/35">
      ...
      >
        <QuizStartPage
          on:start={() => scrollToSection(1)}
          isEdit={$editProfile === "true"}
        />
      </section>
      {#each writingStyle.answers as question, index}
        <section
          id="quiz-section-{index + 1}"
          bind:this={sections[index + 1]}
          class="snap-start h-screen w-full flex items-center justify-center p-4"
        >
          <QuizQuestion
            index={index + 1}
            {question}
            on:valuePicked={updateAnswerAndScroll(index)}
          />
        </section>
      {/each}
      <section
        id="quiz-section-12"
        bind:this={sections[nbOfQuestions + 1]}
        class="snap-start h-screen w-full flex items-center justify-center p-4"
      >
        <QuizEnglishType
          answer={writingStyle.englishType}
  ...
  <div class="absolute bottom-4 left-4 flex flex-col">
    <button
      ...
      class="bg-white/40 text-black p-2 mb-2 rounded-full"
    >
      <svg
    ...
    <button
      id="nextButton"
      on:click={() => scrollToSection(1)}
      class="bg-white/40 text-black p-2 rounded-full"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
