...
export const checkIfAllQuestionsAnswered = (
  writingStyle: WritingStyleDocument | undefined,
) => {
  if (!writingStyle) {
    return false;
  }
  const allQuestionAnswered = writingStyle.answers.every((question) => {
...
export const getWritingPromptFromQuestions = (
  ...
  const personalityArray: string[] = [];
  const toneAndVoiceArray: string[] = [];
